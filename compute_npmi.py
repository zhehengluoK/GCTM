...
def compute_npmi(topics_file, ref_vocab, ref_counts, n_vals, cols_to_skip=0, output_file=None):
    ...
    mean_vals = []
    for n in n_vals:
        mean_npmi = compute_npmi_at_n(topics_file, topics, ref_vocab, ref_counts, n, cols_to_skip=cols_to_skip)
...
def compute_npmi_at_n(topics_file, topics, ref_vocab, ref_counts, n=10, cols_to_skip=0):
    ...
    fh.write_list_to_text(['{:.6f}'.format(np.mean(npmi_means))], os.path.join(topics_file[:len(topics_file)-11], 'npmi.txt'))
