...
def main(args):
    ...
    preprocess_data(train_infile, test_infile, output_dir, train_prefix, test_prefix, min_doc_count, max_doc_freq,
                    vocab_size, stopwords, keep_num, keep_alphanum, strip_html, lower, min_length,
                    label_fields=label_fields)


def preprocess_data(train_infile, test_infile, output_dir, train_prefix, test_prefix, min_doc_count=0, max_doc_freq=1.0,
                    vocab_size=None, stopwords=None, keep_num=False, keep_alphanum=False, strip_html=False, lower=True,
                    min_length=3, label_fields=None):
    if stopwords == 'mallet':
    ...
    word_counts = Counter()
    doc_counts = Counter()  # 每个词在多少个文档中出现过
    count = 0
    ...
    for i, item in enumerate(all_items):
        ...
        text = item['text']
        tokens, _ = tokenize(text, strip_html=strip_html, lower=lower, keep_numbers=keep_num,
                             keep_alphanum=keep_alphanum, min_length=min_length, stopwords=stopword_set, vocab=vocab)
    ...
    # 保存只去掉停用词的文档
    train_only_remove_stopwords = [' '.join(doc) for doc in train_parsed]
    test_only_remove_stopwords = [' '.join(doc) for doc in test_parsed]
    allData_only_remove_stopwords = train_only_remove_stopwords + test_only_remove_stopwords
    fh.write_list_to_text(train_only_remove_stopwords, os.path.join(output_dir, 'train_only_remove_stopwords.txt'))
    fh.write_list_to_text(test_only_remove_stopwords, os.path.join(output_dir, 'test_only_remove_stopwords.txt'))
    fh.write_list_to_text(allData_only_remove_stopwords, os.path.join(output_dir, 'allData_only_remove_stopwords.txt'))

    print("Size of full vocabulary=%d" % len(word_counts))

    print("Selecting the vocabulary")
    most_common = doc_counts.most_common()  # list of tuple
    ...
    train_X_sage, tr_aspect, tr_no_aspect, tr_widx, vocab_for_sage = process_subset(train_items, train_parsed,
                                                                                    label_fields, label_lists, vocab,
                                                                                    output_dir, train_prefix)
    if n_test > 0:
        test_X_sage, te_aspect, te_no_aspect, _, _ = process_subset(test_items, test_parsed, label_fields, label_lists,
                                                                    vocab, output_dir, test_prefix)
...
def process_subset(items, parsed, label_fields, label_lists, vocab, output_dir, output_prefix):
    ...
    for i, words in enumerate(parsed):
        ...
        if len(counter.keys()) > 0:
            ...
            dat_string += ' '.join(
                [str(k) + ':' + str(int(v)) for k, v in zip(list(counter.keys()), list(counter.values()))])
...
def tokenize(text, strip_html=False, lower=True, keep_emails=False, keep_at_mentions=False, keep_numbers=False,
             keep_alphanum=False, min_length=3, stopwords=None, vocab=None):
